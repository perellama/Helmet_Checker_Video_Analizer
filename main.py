    ...
    async def message_handler(input_message):
        if input_message.input_name == "CountInput":
            ...
            if isinstance(inference_list, list) and inference_list:
                ...
                x=len(inference_list)
                y=0
                while y<x:
                    if inference_list[y]['label']=='person':
                         count_ppl+=1
                    if inference_list[y]['label']=='helmet':
                         count_helmet+=1
                    y+=1                   
             
            if count_ppl==count_helmet and count_ppl!=0:
                helmet_checker='YES'   
            print(f'People_Count: {count_ppl}')
            print(f'Date: {now}')
            print(f'Helmet_Count: {count_helmet}')
            print(f'ALL helmets worn: {helmet_checker}')
        

            json_data = {
                    'Date': f'{now}', 
                    'People_Count': count_ppl,
                    'Helmet_Count': count_helmet,
                    'Helmet_Checker': helmet_checker
